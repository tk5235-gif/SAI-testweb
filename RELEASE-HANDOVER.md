...
| 3 | 切替タイミング（アクセスの少ない早朝・深夜を推奨） | 発注者 |
| 4 | リポジトリが**個人アカウント配下**。中長期的には会社Orgへの移管を推奨 | 発注者 |

### コンテンツ面は確定済み（変更不要）

以下は**発注者確認済みの仕様**です。修正提案は不要です。

- **社員インタビュー4本は掲載承認済み。** 文面の変更は行わないでください。
- **採用トップ「人を知る」（3名）とインタビューページ（4名）の人数差は意図的なものです。** 不整合ではないため、揃える必要はありません。
